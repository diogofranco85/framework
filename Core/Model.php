...
class Model{
    ...
    public static function find(int $id, $columns = '*')
    {
        ...
        try{
            ...
            $rs_update = $stmt->fetchObject(get_called_class());
            $rs_update->key_update = $id;
            return $rs_update;
    ...
    {       
        try{
            if($this->softDelete == false){
                ...
                $object = $this->find($this->key_update);
                $column = $this->softDeleteKey;
                $object->$column = 'F';
                $object->update();
                return $object;
            }      
    ...
    public function innerJoin($classname, $idRerefence, $table_reference = null)
    {
        ...
        $idtable = $object->primarykey; 

        $this->joins[] = " INNER JOIN {$table} ON {$table}.{$idtable} = ".$table_reference.".{$idRerefence} ";
    }

    public function rightJoin($classname, $idRerefence, $table_reference = null)
    {
        if(is_null($table_reference))
        {
            $table_reference = $this->tablename;
        ...
        $this->joins[] = " RIGHT JOIN {$table} ON {$table}.{$idtable} = ".$table_reference.".{$idRerefence} ";
    }

    public function leftJoin($classname, $idRerefence, $table_reference = null)
    {
        if(is_null($table_reference))
        {
            $table_reference = $this->tablename;
        ...
        $this->joins[] = " LEFT JOIN {$table} ON {$table}.{$idtable} = ".$table_reference.".{$idRerefence} ";
    }
